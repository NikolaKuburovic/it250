...
if (principals.isEmpty()) {
        ...
        roles.add(user.getUserrola().name());
...
// Null username is invalid
        ...
        Set<String> roles = new HashSet<String>(1);
        roles.add(user.getUserrola().name());
        return new SimpleAuthenticationInfo(user.getUsername(), new String(user.getUserpassword()), getName());
    }
    ...
    private User findByUsername(String username) {
        List<User> users
                = session.createCriteria(User.class).add(Restrictions.eq("username", username)).list();
        if (users.size() > 0) {
            return users.get(0);
        } else {
            return null;
